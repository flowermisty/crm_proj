...
class EventAdminNewController extends BaseController
{
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (!$this->validate($rules, $errors)) {
                ...
                if ($dataResult) {
                    $session = session();
                    $session->setFlashdata('success', 'Successful Registration');
                    alert_move("등록 되었습니다.", "http://godo.event.admin/event_admin_new/init/$event_code");

                } else {
                    alert_move("등록에 실패하였습니다. 데이터에 오류가 있습니다.", "http://godo.event.admin/event_admin_new/init");
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if (!$this->validate($rules, $errors)) {
                ...
                if ($dataResult) {
                    ...
                    alert_move("수정 되었습니다. \\n \\n※직전 수정 항목은 초록색으로 표기 됩니다.", "http://godo.event.admin/event_admin_new/init/{$event_code}");

                } else {
                    alert_move("수정에 실패하였습니다. 데이터에 오류가 있습니다.", "http://godo.event.admin/event_admin_new/update/{$item_code}/{$event_code}");
    ...
    public function delete()
    {
        ...
        if ($this->request->getMethod() == 'post' && $deletePack) {
            ...
            alert_move("삭제 되었습니다.", "http://godo.event.admin/event_admin_new/init/{$this->request->getVar('event_delete')}");
            return;
        }
